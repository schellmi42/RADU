...
print('## BUILDING: RADU model at input resolution: ###')
...
def model_params(string):
  ...
  """
  # if string == 'v1':
  #   feature_sizes = [4, 8, 256, 128, 64]
  #   conv_radius = [0.1, 0.1, 0.1]
  #   sampling_rate = 8
...
class mymodel(tf.Module):
  ...
  Args:
    ...
    layer_type: A `string`, the type of convolution used,
      can be 'MCConv', 'MCConv_radu'.
    sampling_rate: `int`, sampling rate for spatial downscaling.
  '''

  def __init__(self,
               feature_sizes,
               conv_radius,
               layer_type='MCConv_radu',
               sampling_rate=8,
    ...
    if self.layer_type == 'MCConv_radu':
  ...
  def __call__(self,
    ...
    # prepare point cloud
    sizes = tf.ones([self.batch_size], dtype=tf.int32) * tf.math.reduce_prod(self.sampling_shape)
    sampled_points = tf.reshape(sampled_points, [self.batch_size * tf.math.reduce_prod(self.sampling_shape), 3])
    ...
    features = tf.reshape(features, [-1, self.feature_sizes_3D[0]])

    # 3D network
    if self.layer_type == 'MCConv_radu':
      additional_args = {'rays': tf.reshape(sampled_rays, [self.batch_size * tf.math.reduce_prod(self.sampling_shape), 3])}
    else:
      additional_args = {}
    for i in range(self.num_levels_3D):
